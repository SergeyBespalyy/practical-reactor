...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void behold_flatmap() {
        ...
        Mono<String> currentUserEmail = getCurrentUser() // Get the currently logged-in user
                .flatMap(user -> getUserEmail(user)); // Retrieve the email for the given user
...
//                .concatWith(numberService2());
        ...
        Flux<Integer> numbers = Flux.just(numberService1(), numberService2()) // Создать Flux из сервисов
                .flatMapSequential(service -> service); // Последовательно объединить
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary()
                .switchOnFirst((signal, flux) -> {
                    if (signal.hasValue() && !Objects.requireNonNull(signal.get()).metaData.equals("spam")) {
                        return flux;
                    }
    ...
    public void instant_search() {
        ...
        Flux<String> suggestions = userSearchInput()
                .switchMap(input -> autoComplete(input)); // Переключиться на новый запрос и отменить предыдущий
    ...
    public void prettify() {
        ...
        Mono<Boolean> successful = Mono.when(
                        openFile(),
    ...
    public void one_to_n() {
        //todo: feel free to change code as you need
        Flux<String> fileLines = Flux.firstWithSignal(openFile())
                .thenMany(readFile());
...
//                .index()
//                .concatMap(tuple -> {
//                    Long taskIndex = tuple.getT1(); // Получаем индекс
//                    return commitTask(String.valueOf(taskIndex)) // Выполняем фиксацию с использованием индекса
//                            .thenReturn(taskIndex); // Возвращаем индекс для обработки далее
//                })
//                .map(index -> "task#" + (index + 1));

        Flux<String> committedTasksIds = tasksToExecute()
                .concatMap(taskMono -> taskMono // Развернуть Flux<Mono<String>> во внутренний Mono<String>
                        .flatMap(task -> commitTask(task) // Выполнить фиксацию для задачи
                                .thenReturn(task) // Вернуть задачу для обработки далее
                        )
                )
                .map(task -> task); // Создать идентификатор задачи
